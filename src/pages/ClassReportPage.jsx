...
// Distinct background colours for each learning area â€” up to 12 areas supported
const AREA_PALETTE = [
  ...
  { bg: '#E0F7FA', text: '#006064', border: '#80DEEA' }, // teal
  { bg: '#FFFDE7', text: '#F57F17', border: '#FFF176' }, // yellow
  { bg: '#FBE9E7', text: '#BF360C', border: '#FFAB91' }, // deep orange
  { bg: '#E8EAF6', text: '#1A237E', border: '#9FA8DA' }, // indigo
  { bg: '#F1F8E9', text: '#33691E', border: '#C5E1A5' }, // light green
  { bg: '#E0F2F1', text: '#004D40', border: '#80CBC4' }, // teal 2
  { bg: '#FFF8E1', text: '#FF6F00', border: '#FFE082' }, // amber
];

// PDF RGB versions of the area palette backgrounds (light tints for data cells)
...
function LevelBadge({ level, pct }) {
  if (!level) return <span style={{ color: '#ccc', fontSize: 11 }}>â€”</span>;
...
export default function ClassReportPage() {
  ...
  // â”€â”€ CBC performance-level colour helper â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function levelColor(level) {
    return LEVEL_COLOR[level] || { bg: '#F5F5F5', text: '#888', border: '#DDD' };
  }

  // â”€â”€ Competency rendering â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      // â”€â”€ column geometry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      // â”€â”€ area name abbreviation (max 8 chars) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      function abbr(name) {
        if (!name) return '';
      ...
      function drawPageHeader(doc, y) {
        ...
        doc.text(`CBC Class Report  â€”  ${cls.class_name}  Â·  ${report.term}  Â·  ${cls.academic_year}`, ML + 2, y + 4.8);
        doc.setTextColor(200, 200, 200);
        doc.setFontSize(7);
        doc.text(`Students: ${students.length}  Â·  Sessions: ${sessions.length}`, PW - MR - 2, y + 4.8, { align: 'right' });
        doc.setTextColor(0, 0, 0);
        return y + 9;
      }

      // â”€â”€ two-row table header: Row1=Sessions, Row2=Areas â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      function drawTableHeader(doc, y) {
        ...
        // Row 1 â€” Session group headers (each spans areas.length columns)
        let cx = ML + nameW;
        ...
        // Row 2 â€” Area sub-headers repeated under each session, each area has its own colour
        y += row1H;
        cx = ML + nameW;
        for (const sess of sessions) {
      ...
      // â”€â”€ draw a student row â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      const ROW_H = 5.5;
      function drawStudentRow(doc, st, y, isEven) {
        ...
        const nameStr = st.full_name.length > 26 ? st.full_name.slice(0, 25) + 'â€¦' : st.full_name;
        doc.text(nameStr, ML + 2, y + ROW_H / 2 + 1.8);

        // Data cells â€” sessions outer loop, areas inner
        // Each cell gets a light area-colour background tint + bold EE/ME/AE/BE text
        let cx = ML + nameW;
        for (const sess of sessions) {
          for (let ai = 0; ai < areas.length; ai++) {
            const area = areas[ai];
            ...
            const ac = AREA_PALETTE_PDF[ai % AREA_PALETTE_PDF.length];
            // Always draw the area tint background
            doc.setFillColor(ac.bg[0], ac.bg[1], ac.bg[2]);
            ...
            if (level) {
              ...
              doc.setFont('helvetica', 'normal');
              doc.setFontSize(6);
              doc.text('â€”', cx + cellW / 2, y + ROW_H / 2 + 1.8, { align: 'center' });
            }
            cx += cellW;
          }
        }

        // Overall â€” colour fill only, no text
      ...
      // â”€â”€ legend: level key + area colour key â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      function drawLegend(doc, y) {
        ...
        for (let ai = 0; ai < areas.length; ai++) {
          ...
          lx += 18;
          if (lx > PW - MR - 20) break; // safety overflow guard
        }
        doc.setTextColor(0, 0, 0);
      ...
      function drawFooter(doc, pageNum, totalPages) {
        ...
        doc.text(`Generated by Smarternow Data Venture Â· Page ${pageNum} of ${totalPages}`, PW / 2, PH - 4, { align: 'center' });
        doc.setTextColor(0, 0, 0);
      }

      // â”€â”€ build all pages â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      <div style={{ backgroundColor: '#fff', padding: '12px 16px', borderBottom: '1px solid #E0E0E0', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button onClick={() => navigate('/home')} style={{ background: 'none', border: 'none', color: '#7B4F9B', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>â† Back</button>
          ...
          <>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
              {[
                ...
                ['Term', `${report.term} Â· ${report.class.academic_year}`],
            ...
            {sessions.length === 0 ? (
              ...
              <>
                ...
                {/* â”€â”€ Main CBC matrix table â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
                {/* Columns: Student | [Area Ã— Session]â€¦ | Overall */}
                  ...
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ borderCollapse: 'collapse', fontSize: 12, width: '100%' }}>
                      <thead>
                          ...
                          {sessions.map((sess, si) => (
                            <th key={sess.session_id} colSpan={areas.length}
                              ...
                              <span style={{ fontSize: 9, fontWeight: 400, marginLeft: 4, opacity: 0.8 }}>
                                {sess.status === 'Open' ? 'ðŸŸ¢' : sess.status === 'Closed' ? 'ðŸ”’' : 'â³'}
                              </span>
                            </th>
                          ))}
                        ...
                        {/* Row 2: Area sub-headers â€” each area has its own colour */}
                        <tr style={{ backgroundColor: '#FAFAFA' }}>
                          {sessions.map(sess => (
                            areas.map((area, ai) => {
                ...
                {/* â”€â”€ Per-session level distribution â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
                    ...
                    {sessions.map(sess => {
                      ...
                      return (
                        <div key={sess.session_id} style={{ border: '1px solid #EEE', borderRadius: 10, padding: 12 }}>
                          ...
                          <div style={{ fontSize: 10, color: '#AAA', marginBottom: 8 }}>{sess.exam_name} Â· {stats.assessed} assessed</div>
                          {['EE','ME','AE','BE'].map(l => {
                ...
                {/* â”€â”€ Competencies â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
